...
export default function SelectablePart({ part }) {
  ...
  const handleSelectCooler = (part) => {
    EventEmitter.emit("coolerSelected", part);
    router.back();
  };
    ...
    <View style={styles.part}>
      <View style={[styles.topContainer, dynamicStyles.topContainer]}>
        ...
        <AnimatedIconButton
          ...
          iconName="diff-added"
          buttonText=""
          onPress={() => handleSelectCooler(part)}
      ...
      <View style={styles.bottomContainer}>
        ...
        <View style={styles.infoContainer}>
          ...
          <Text style={styles.partText}>{part.rpm}rpm</Text>
        </View>
        <View style={styles.infoContainer}>
          <Text style={styles.label}>Noise level</Text>
          <Text style={styles.partText}>{part.noise_level}dB</Text>
        </View>
        {part.radiator_size > 0 && part.cooler_height <= 0 ? (
          <View style={styles.infoContainer}>
            <Text style={styles.label}>Radiator size</Text>
            <Text style={styles.partText}>{part.radiator_size}mm</Text>
          </View>
        ) : part.cooler_height > 0 && part.radiator_size <= 0 ? (
          <View style={styles.infoContainer}>
            <Text style={styles.label}>Cooler height</Text>
            <Text style={styles.partText}>{part.cooler_height}mm</Text>
          </View>
        ) : (
          <View style={styles.infoContainer}>
            <Text style={styles.label}>Radiator size/Cooler height</Text>
            <Text style={styles.partText}>Data error</Text>
          </View>
        )}
      </View>
    </View>
