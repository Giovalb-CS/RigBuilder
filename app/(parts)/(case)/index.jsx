...
import SelectableCase from "../../../components/parts/SelectableCase";
...
export default function CaseScreen() {
  ...
  const [minCoolerHeight, setMinCoolerHeight] = useState(
    params.minCoolerHeight || ""
  );
  const [selectedMinRadiatorSize, setMinRadiatorSize] = useState(
    params.minRadiatorSize || ""
  );
  const [minGPULength, setMinGPULength] = useState(params.minGPULenght || "");
  const [selectedFormFactor, setSelectedFormFactor] = useState(
    params.formFactor || ""
  );
  const [minPSULength, setMinPSULength] = useState(params.minPSULenght || "");
  const [minPCIeSlots, setMinPCIeSlots] = useState(params.minPCIeSlots || "");
  ...
  const postCase = async () => {
    try {
      ...
      const filters = {
        ...
        ...(selectedMinRadiatorSize && {
          minRadiatorSize: selectedMinRadiatorSize,
        }),
      ...
      const response = await fetch(API_URL + "/cases/filters", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-API-KEY": API_KEY,
        ...
        >
          ...
          <View style={[{ width: "100%" }, styles.filterSection]}>
            <TextInput
              ...
              placeholder="Enter a case brand or model"
              value={nameFilter}
            ...
            <View style={styles.advancedForm}>
              ...
              <View style={[styles.selectContainer]}>
                <View style={styles.pickerContainer}>
                  <Picker
                    selectedValue={selectedMinRadiatorSize}
                    onValueChange={setMinRadiatorSize}
                    style={styles.picker}
                    onPressIn={() => setFocusedInput("minRadiatorSize")}
                    onEndEditing={() => setFocusedInput(null)}
                    dropdownIconColor={Colors.theme.white}
                  >
                    <Picker.Item label="Min Radiator size" value="" />
              ...
              <TextInput
                ...
                value={minGPULength}
                onChangeText={setMinGPULength}
                keyboardType="numeric"
                style={[
                  styles.input,
                  focusedInput === "minGPULength" && {
                    borderColor: Colors.theme.orange,
                  },
                ]}
                onPressIn={() => setFocusedInput("minGPULength")}
                onEndEditing={() => setFocusedInput(null)}
                placeholderTextColor={
                  focusedInput === "minGPULength"
                    ? Colors.theme.white
              ...
              <TextInput
                cursorColor={Colors.theme.orange}
                placeholder="Min PSU Length"
                value={minPSULength}
                onChangeText={setMinPSULength}
                keyboardType="numeric"
                style={[
                  styles.input,
                  focusedInput === "minPSULength" && {
                    borderColor: Colors.theme.orange,
                  },
                ]}
                onPressIn={() => setFocusedInput("minPSULength")}
                onEndEditing={() => setFocusedInput(null)}
                placeholderTextColor={
                  focusedInput === "minPSULength"
                    ? Colors.theme.white
              ...
              <TextInput
                cursorColor={Colors.theme.orange}
                placeholder="Min PCIe Slots"
                value={minPCIeSlots}
                onChangeText={setMinPCIeSlots}
                keyboardType="numeric"
                style={[
                  styles.input,
                  focusedInput === "minPCIeSlots" && {
                    borderColor: Colors.theme.orange,
                  },
                ]}
                onPressIn={() => setFocusedInput("minPCIeSlots")}
                onEndEditing={() => setFocusedInput(null)}
                placeholderTextColor={
                  focusedInput === "minPCIeSlots"
                    ? Colors.theme.white
        ...
        {error ? (
          ...
          <>
            <AnimatedFlashList
              ...
              renderItem={({ item }) => <SelectableCase part={item} />}
